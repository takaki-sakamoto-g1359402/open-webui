...
export class DummyEnemy {
  constructor(scene, world, spawnX, spawnZ) {
    this.scene = scene;
    this.world = world;
    this.spawnX = spawnX;
    this.spawnZ = spawnZ;
    ...
    this.body = BABYLON.MeshBuilder.CreateCylinder("dummy-body", { diameter: 1.15, height: 2.4, tessellation: 6 }, scene);
    this.body.parent = this.root;

    this.ring = BABYLON.MeshBuilder.CreateTorus("dummy-ring", { diameter: 1.9, thickness: 0.12, tessellation: 24 }, scene);
  ...
  update(deltaSeconds, elapsedSeconds) {
    ...
    if (this.flashTimer > 0) {
      ...
      this.material.emissiveColor.set(0.08 + (1 - hpFactor) * 0.18, 0.22 * hpFactor, 0.42 * hpFactor);
    }
  }

  segmentHitDistance(segmentStart, segmentEnd, padding = 0) {
    if (!this.alive) return null;
    const closest = BABYLON.Vector3.Lerp(
      segmentStart,
      segmentEnd,
      clamp(
        BABYLON.Vector3.Dot(this.root.position.subtract(segmentStart), segmentEnd.subtract(segmentStart)) /
  ...
  applyDirectHit(amount) {
    this.takeDamage(amount);
  }

  applyExplosion(point, radius, maxDamage) {
  ...
  getStatusText() {
    if (!this.alive) return `Dummy: rebuilding (${this.respawnTimer.toFixed(1)}s)`;
