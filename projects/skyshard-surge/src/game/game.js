...
import { DummyEnemy } from "./entities/dummy-enemy.js";
...
export class SkyshardGame {
  ...
  async initialize() {
    ...
    this.dummy = new DummyEnemy(this.scene, this.world, 16, 14);
    this.projectiles = new ProjectileSystem(this.scene, this.world, this.player, [this.dummy]);

    this.selectionMesh = BABYLON.MeshBuilder.CreateBox("selection-box", { size: 1.03 }, this.scene);
  ...
  setupEnvironment() {
    const sun = new BABYLON.DirectionalLight("sun", new BABYLON.Vector3(-0.45, -1, -0.35), this.scene);
    sun.intensity = 1.1;
    sun.diffuse = new BABYLON.Color3(1, 0.96, 0.91);

    const skyLight = new BABYLON.HemisphericLight("sky-light", new BABYLON.Vector3(0.2, 1, 0.1), this.scene);
    skyLight.intensity = 0.55;
    skyLight.groundColor = new BABYLON.Color3(0.16, 0.19, 0.24);

    const shard = BABYLON.MeshBuilder.CreatePolyhedron("floating-shard", { type: 1, size: 4.3 }, this.scene);
    ...
    const halo = BABYLON.MeshBuilder.CreateTorus("floating-halo", { diameter: 8.5, thickness: 0.16, tessellation: 40 }, this.scene);
  ...
  update(deltaSeconds) {
    ...
    this.currentTarget = this.world.raycast(this.player.getEyePosition(), this.player.getForwardVector(), CONFIG.world.maxTargetDistance);

    if (this.currentTarget) {
      this.selectionMesh.position.set(this.currentTarget.voxel.x + 0.5, this.currentTarget.voxel.y + 0.5, this.currentTarget.voxel.z + 0.5);
    ...
    if (this.currentTarget && (this.input.wasPressed("KeyR") || this.input.wasPressed("Mouse2"))) {
      const removed = this.world.setBlock(this.currentTarget.voxel.x, this.currentTarget.voxel.y, this.currentTarget.voxel.z, 0);
      if (removed) this.world.rebuildAllDirtyChunks();
    }
    ...
    if (this.currentTarget && this.input.wasPressed("KeyE")) {
      ...
      if (this.world.canPlaceBlock(placePosition.x, placePosition.y, placePosition.z, bounds)) {
        const placed = this.world.setBlock(placePosition.x, placePosition.y, placePosition.z, this.world.placeableBlockId);
        if (placed) this.world.rebuildAllDirtyChunks();
      }
    }

    if (this.input.wasPressed("Mouse0") || this.input.wasPressed("KeyG")) {
      this.projectiles.tryFire();
    }

    this.projectiles.update(deltaSeconds);
    this.dummy.update(deltaSeconds, this.elapsedSeconds);
  ...
  updateHud() {
    const targetText = this.currentTarget
      ? `${this.world.getBlockDisplayName(this.world.getBlock(this.currentTarget.voxel.x, this.currentTarget.voxel.y, this.currentTarget.voxel.z))} @ (${this.currentTarget.voxel.x}, ${this.currentTarget.voxel.y}, ${this.currentTarget.voxel.z})`
      : "Target: none";

    const statusText = this.input.isPointerLocked()
      ? `Flight ${this.player.flightEnabled ? "ON" : "OFF"} | Dash ${this.player.dashCooldown.toFixed(1)}s | Click fire, Q dash`
    ...
    this.hud.update({
      ...
      enemyText: this.dummy.getStatusText(),
  ...
  getDebugState() {
    return {
      ...
      world: {
        ...
        target: this.currentTarget ? { x: this.currentTarget.voxel.x, y: this.currentTarget.voxel.y, z: this.currentTarget.voxel.z } : null,
      },
      dummy: this.dummy.getDebugState(),
      combat: { activeProjectiles: this.projectiles.getActiveProjectileCount() },
      camera: {
        position: {
