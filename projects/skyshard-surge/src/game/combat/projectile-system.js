...
export class ProjectileSystem {
  constructor(scene, world, player, enemies) {
    this.scene = scene;
    this.world = world;
    this.player = player;
    ...
    for (let index = 0; index < CONFIG.combat.projectilePoolSize; index += 1) {
      const mesh = BABYLON.MeshBuilder.CreateSphere(`projectile-${index}`, { diameter: 0.28, segments: 6 }, scene);
      mesh.material = this.projectileMaterial;
      mesh.isPickable = false;
      mesh.setEnabled(false);
      this.projectiles.push({ active: false, mesh, position: new BABYLON.Vector3(), direction: new BABYLON.Vector3(), lifetime: 0 });
    }
  }

  update(deltaSeconds) {
    if (this.fireCooldown > 0) this.fireCooldown = Math.max(0, this.fireCooldown - deltaSeconds);
    ...
    for (const projectile of this.projectiles) {
      ...
      if (projectile.lifetime <= 0) this.deactivateProjectile(projectile);
    }
  ...
  tryFire() {
    ...
    if (!this.player.consumeEnergy(CONFIG.resources.projectileEnergyCost)) return false;
  ...
  getClosestEnemyHit(segmentStart, segmentEnd) {
    let bestHit = null;
    for (const enemy of this.enemies) {
      ...
      if (!bestHit || distance < bestHit.distance) {
        ...
        bestHit = { enemy, distance, point: segmentStart.add(direction.scale(distance)) };
      }
    }
    return bestHit;
  }

  applyExplosion(point) {
    const removedCount = this.world.removeBlocksInRadius(point, CONFIG.combat.projectileRadius);
    if (removedCount > 0) this.world.rebuildAllDirtyChunks();
    for (const enemy of this.enemies) {
  ...
  createImpact(position) {
    const mesh = BABYLON.MeshBuilder.CreateSphere("impact", { diameter: 0.6, segments: 8 }, this.scene);
    mesh.material = this.impactMaterial.clone(`impact-material-${this.impacts.length}`);
    mesh.position.copyFrom(position);
    mesh.isPickable = false;
    this.impacts.push({ mesh, remaining: 0.18, duration: 0.18 });
  }
