...
- Release preflight: run `pnpm check:release-preflight -- --strict-production --env-file .env.production.local` before claiming public production readiness so missing external tools, demo env values, Vercel cron drift, or stale screenshot evidence are reported in one place.
