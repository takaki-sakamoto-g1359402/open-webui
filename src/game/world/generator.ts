...
export class VoxelGenerator {
  ...
  fractalNoise(worldX: number, worldZ: number, baseScale: number, octaves: number): number {
    let total = 0;
    let amplitude = 1;
    let frequency = 1;
    let normalization = 0;
  ...
  valueNoise(worldX: number, worldZ: number, scale: number, localSeed: number): number {
