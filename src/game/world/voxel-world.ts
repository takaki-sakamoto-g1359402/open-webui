...
export class VoxelWorld {
  ...
  constructor(scene: any, physicsWorld: any) {
    this.scene = scene;
  ...
  getBlockDisplayName(blockId: number): string {
  ...
  setBlock(worldX: number, worldY: number, worldZ: number, blockId: number): boolean {
    ...
    if (localX === CONFIG.world.chunkSize - 1) {
      this.markChunkDirty(chunkX + 1, chunkZ);
    }
    if (localZ === 0) {
      this.markChunkDirty(chunkX, chunkZ - 1);
    }
    if (localZ === CONFIG.world.chunkSize - 1) {
      this.markChunkDirty(chunkX, chunkZ + 1);
    }
  ...
  removeBlocksInRadius(center: any, radius: number): number {
    ...
    for (let x = Math.floor(center.x - radius); x <= Math.ceil(center.x + radius); x += 1) {
      for (let y = Math.floor(center.y - radius); y <= Math.ceil(center.y + radius); y += 1) {
        for (let z = Math.floor(center.z - radius); z <= Math.ceil(center.z + radius); z += 1) {
          ...
          if (!isDestructibleBlock(blockId, y)) {
            continue;
          }

          if (this.setBlock(x, y, z, BLOCK_IDS.AIR)) {
            removedCount += 1;
          }
        }
      }
  ...
  canPlaceBlock(worldX: number, worldY: number, worldZ: number, bounds: any): boolean {
    if (this.getBlock(worldX, worldY, worldZ) !== BLOCK_IDS.AIR) {
      return false;
    }
  ...
  raycast(origin: any, direction: any, maxDistance: number): any {
    ...
    while (distance <= maxDistance) {
      if (isSolidBlock(this.getBlock(x, y, z))) {
        return {
          voxel: new BABYLON.Vector3(x, y, z),
          normal,
      ...
      if (tMaxX < tMaxY && tMaxX < tMaxZ) {
        ...
        distance = tMaxX;
        tMaxX += invX;
        normal = new BABYLON.Vector3(-stepX, 0, 0);
      } else if (tMaxY < tMaxZ) {
        y += stepY;
        distance = tMaxY;
        tMaxY += invY;
        normal = new BABYLON.Vector3(0, -stepY, 0);
      } else {
        z += stepZ;
        distance = tMaxZ;
        tMaxZ += invZ;
        normal = new BABYLON.Vector3(0, 0, -stepZ);
      }
    }
  ...
  getLocalIndex(localX: number, y: number, localZ: number): number {
    return y * CONFIG.world.chunkSize * CONFIG.world.chunkSize + localZ * CONFIG.world.chunkSize + localX;
  }

  ensureChunk(cx: number, cz: number): any {
    ...
    const chunk = {
      ...
      cx,
      cz,
      blocks: this.generator.generateChunk(cx, cz),
      mesh: null,
      dirty: true,
    };
  ...
  rebuildChunk(chunk: any): void {
    ...
    chunk.mesh.position.set(
      chunk.cx * CONFIG.world.chunkSize,
      0,
      chunk.cz * CONFIG.world.chunkSize,
    );

    this.physicsWorld.setChunkCollider(
      chunk.key,
      meshData.positions,
      meshData.indices,
      chunk.mesh.position,
