...
async def test_generate_redeem():
    async with AsyncClient(app=app, base_url="http://test") as ac:
        ...
        res = await ac.post(
            "/auth/register",
            json={"email": "admin@test", "password": "pw", "id_document": "id"},
        )
        ...
        # generate invite
        res2 = await ac.post(
            "/invites/generate", headers={"Authorization": f"Bearer {token_admin}"}
        )
        code = res2.json()["code"]
        # register member
        res3 = await ac.post(
            "/auth/register",
            json={"email": "u@test", "password": "pw", "id_document": "id"},
        )
